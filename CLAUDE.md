...
Decentraland's main website. Single Vite SPA that absorbed several standalone dapps: the homepage and legal pages, the download flow, `/whats-on/*` (events), `/blog/*` (CMS posts), `/jump/*` (deep-link handler for the launcher), `/discover/*` (live scenes + communities, absorbs the standalone decentraland.social experience), `/cast/*` (LiveKit browser streaming), `/storage/*` (storage-service-site), `/reels/*` (in-game camera screenshots), and `/report/*` (community report flow). Module Federation was removed — every absorbed dapp is a native lazy-loaded route group in this repo.
...
- **Discover** (absorbs the standalone decentraland.social experience): `/discover` (LIVE NOW hot scenes + BROWSE places/worlds with category/search filters), `/discover/communities` (list), `/discover/communities/:id` (detail), `/discover/place/:position` (Genesis City parcel detail with inline LiveKit watcher + chat), `/discover/world/:name` (world detail, same shape). All `/discover/*` sub-routes render inside the shared `<DiscoverLayout />` (top tab nav: DISCOVER / COMMUNITIES). Catch-all `/discover/*` → `DiscoverNotFoundPage`.
  - **Scene detail layout**: 2-column grid (hero column = watcher card + info card stacked; right column = permanent chat sidebar with **fixed pixel height** and internal scroll — 640px desktop / 420px mobile). No events section.
  - **`<SceneLiveWatcher />`** (`src/components/discover/SceneLiveWatcher/`) is split into a `useSceneRoom` hook (`src/hooks/useSceneRoom.ts` — owns credential fetching) + `<SceneRoomMount />` (wraps in `LiveKitProvider` + `LiveKitRoom` + `ChatProvider`) + `<SceneWatcherCard />` and `<SceneChatDock />` display components. Token paths tried in order: (1) **`/get-scene-adapter`** signed-fetch as a guest identity minted in-browser (`features/discover/guestIdentity.ts` + `features/discover/sceneAdapter.ts`) — joins the FULL scene comms LiveKit room (presence, voice, chat, any publisher including OBS/RTMP/WHIP); (2) fallback to **`/cast/watcher-token`** (cast2 scene-stream-access path) when no guest auth.
  - **Env-driven LiveKit calls**: gatekeeper / peer / worlds-content-server URLs come from `getEnv('GATEKEEPER_URL')` / `getEnv('PEER_URL')` / `getEnv('WORLDS_CONTENT_SERVER_URL')` with prod fallbacks. `dev.json` and `prd.json` point at `.org`; `stg.json` points at `.zone`. The `.zone` deployment also loads `dev.json` (per `@dcl/ui-env` TLD mapping) and intentionally hits prod data — `.zone` has no equivalent live cluster.
  - **Mobile not-supported**: bevy-web doesn't run on touch devices. `SceneWatcherReady` detects mobile via `useAdvancedUserAgentData`, gates the bevy iframe mount entirely, and renders a "Decentraland Web" card with the App Store / Google Play badge (same `StoreBadgeLink` styled the landing Hero uses).
  - **Iframe `credentialless` attribute**: required for the bevy iframe to inherit the parent's COEP credentialless context and become `crossOriginIsolated` (bevy's SharedArrayBuffer worker postMessage errors `DataCloneError` otherwise). Set via a ref callback (`useCredentiallessIframeRef`) because React's JSX spread of unknown HTML attributes was being stripped in the built bundle. COOP `same-origin` + COEP `credentialless` are set per-route in `vercel.json` for `/discover/:type(place|world)/:path*`.
...
- **Creators** (Roblox-style creator dashboard for Worlds): `/creators` (Creations grid — worlds the connected wallet owns or collaborates on, enriched with places-api thumbnails + live user counts). `/creators/world/:name` renders `<CreatorWorldLayout />` (a routed left rail — each tab is its own page, not in-page scrolling) wrapping these sub-routes: index `OverviewPage` (clean hero + a Deployment info card), `preview/` (minimal `PreviewPage` — a poster that, on a single click, mounts the bevy scene-viewer iframe directly with a `credentialless` ref + a fullscreen-only control; NO LiveKit/`SceneWatcherCard` — bevy handles its own comms. Needs COEP credentialless, so `vercel.json` sets COOP `same-origin` + COEP `credentialless` for `/creators` and `/creators/:path*`, same as `/discover`), `deploys/` (active deployment(s) — worlds-content-server keeps no deploy history, so this lists the current deployment per scene: when/deployer/entity id), `storage/` (the full sites/storage manager embedded, scoped to `realm=<world>` — see below), `bans/` (scene moderation — see below), and `analytics/` + `monetization/` "Coming soon" placeholders. The layout fetches once and passes data to sub-pages via `useWorldContext` (Outlet context). Catch-all `/creators/*` → `CreatorsNotFoundPage`, all inside `<CreatorsLayout />`. Data layer: `src/features/creators/` injects `getWorldDeployments` into `placesClient` (fetches `/world/{name}/scenes`); reuses `useGetUserDCLNamesQuery` + `useGetContributableDomainsQuery` (storage) for the list and `useGetDiscoverWorldsByNamesQuery` (discover) for enrichment.
  - **Gatekeeper scene-management tabs** (`gatekeeperClient` base in `src/services/`, signed-fetch with `signedFetchFactory`). Shared helper `src/features/sceneGatekeeper/sceneGatekeeper.helpers.ts` (`sceneSignedFetch` + `sceneMetadata` + `SceneScope`) — **crucial gotcha**: these endpoints validate via the gatekeeper's `validate()`, which reads a **`realm` OBJECT** from the signed metadata (`{ serverName: <worldName>, hostname: <worlds-content-server-url> }`, `isWorld` ← hostname contains `worlds-content-server`) — NOT the `realmName` string `/get-scene-adapter`'s `validateSceneAdmin()` uses. Tabs: **Bans** (`src/features/bans/`, `/scene-bans`, ban by `banned_address` or `banned_name`), **Scene admins** (`src/features/sceneGatekeeper/admins.client.ts`, `/scene-admin`, add by `admin`/`name`; implicit admins like the owner return `canBeRemoved:false`), **Streaming** (`streaming.client.ts`, `/scene-stream-access` GET/POST/PUT-reset/DELETE → OBS `streaming_url`+`streaming_key`, 4-day TTL). Bans + Scene-admins share the `entryList.styled.ts` address-list UI.
  - **Access tab** (`src/features/worldPermissions/` → injected into `storageClient`): the world ACL via worlds-content-server `/world/:name/permissions` (public GET) + signed `PUT`/`DELETE /world/:name/permissions/:permission/:address` (`deployment`/`streaming`/`access`). Reuses storage's `createScopedSignedFetch`+`sendSignedFetch` (wallet-scoped, no scene metadata). `access` can be unrestricted/nft/shared-secret (no managed wallet list) vs. allow-list.
  - **Storage gate**: the scene/player storage service only works when scene.json sets `authoritativeMultiplayer: true` (readable from `/world/:name/scenes` → `entity.metadata.authoritativeMultiplayer`, surfaced on `WorldDeployment`). `StoragePage` shows an "enable it in scene.json" message instead of empty managers when it's absent.
  - **Shared storage managers**: `src/components/storage/managers/` (`EnvManager`, `SceneManager`, `PlayersManager`) hold the env/scene/players CRUD bodies, parameterized by `{ identity, realm, position }`. Both the `/storage/*` pages (`EnvPage`/`ScenePage`/`PlayersPage`, now thin `<StorageLayout>` wrappers) and the creators `StoragePage` render them — one implementation, two surfaces.
...
| `src/pages/`                     | Page components. `whats-on/`, `blog/`, `jump/`, `discover/`, `cast/`, `storage/`, `reels/`, `report/` subdirs for the absorbed apps.                                                                                                                                                                                                                                                                                                                                                                    |
| `src/pages/index.tsx`            | Landing homepage (hero is prerendered by `scripts/prerender-hero.mjs`).                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/components/`                | Shared components. Top-level for landing-specific UI. `blog/`, `whats-on/`, `jump/`, `discover/`, `cast/`, `storage/`, `Reels/`, `Report/`, `Create/`, `Invite/`, `Support/` subdirs for the absorbed apps and themed sections.                                                                                                                                                                                                                                                                         |
...
| `src/features/communities/`      | Everything that hits the Social API. RTK Query endpoints + helpers for `/discover/communities/*` injected into `socialClient` (the backend service is still named "social-api").                                                                                                                                                                                                                                                                                                                        |
| `src/features/discover/`         | Aggregates the data feeding the `/discover/*` routes. Injects `getDiscoverPlaces`, `getDiscoverWorlds`, `getHotScenes`, `getLiveWorlds`, `getDiscoverPlaceByPosition`, `getDiscoverWorldByName`, `getDiscoverWorldsByNames` into `placesClient`, and `getCommunitiesList` into `socialClient`. Holds the category list (`DISCOVER_CATEGORIES`), position parsing, the `decentraland://` deep-link builder, and the scene-adapter / cast-watcher token fetchers (`sceneAdapter.ts`, `guestIdentity.ts`). |
...
- **DappsShell is lazy** — Redux, RTK Query, Contentful renderer, dompurify, livekit-client (`@livekit/components-react`) and the per-dapp endpoint code only load when a user navigates to `/whats-on/*`, `/blog/*`, `/jump/*`, `/discover/*`, `/cast/*`, or `/storage/*`.
...
All env vars live in `src/config/env/{dev,stg,prd}.json`. Access via `getEnv('KEY')` from `src/config/env.ts`. The `@dcl/ui-env` package auto-selects the right file based on the hostname. Override at runtime with the `?env=dev|stg|prod` query param — note the value is the `@dcl/ui-env` **Env value** (`dev` / `stg` / `prod`), NOT the JSON filename: `?env=prod` (not `prd`) loads `prd.json` (the `.org` cluster). `@dcl/ui-env` resolves env in this order: query param → hostname TLD (`.org`→prod, `.zone`/`.io`→dev) → `VITE_REACT_APP_DCL_DEFAULT_ENV` default (`dev`). On `localhost` only the query param works, so use `?env=prod` to preview against prod data.
...
1. Create page in `src/pages/<area>/`. Mirror the existing absorbed-dapp layout: `whats-on/`, `blog/`, `jump/`, `discover/`, `cast/`, `storage/`.
2. Add `lazy(() => import(...))` in `src/App.tsx`.
...
- **Pages**: `src/pages/<route>/`. Heavy routes under `src/pages/{whats-on,blog,jump,discover,cast,storage}/`. Layout-less fullscreen routes use the same `src/pages/<area>/` shape but are placed before the `<Layout />` Route block in `src/App.tsx` (`reels`, `download`, `invite`).
