...
- **26. `@dcl/schemas` is the source of truth for shared marketplace tokens.** Rarity colors come from `Rarity.getColor(rarity)` + `Rarity.getGradient(rarity)`. Network endpoints, category enums, max-supply tables — same. Never hand-code a `RARITY_COLOR_MAP`-style literal in sites or in ui2; derive from schemas (or theme palette if it already wraps schemas). When in doubt: `grep "from '@dcl/schemas'"` for prior art before inventing a constant.
...
- Same anti-stacking pattern for **photos / places / communities** — `useOpenPhotoModal`, `useOpenPlaceModal`, `useOpenCommunityModal`. Each delegates to the provider when one is mounted (so the event/profile modal swaps in-place — `event → profile → (photo | place | community)` with back-chevron traversal), or opens a standalone Dialog (`PhotoModal` / `PlaceDetailModal` / `CommunityDetailModal`) otherwise. **Never render a Dialog from inside a tab when one of these hooks is available.**
- `ProfileSurface` accepts `embedded?: boolean` — strip the outer chrome when mounted inside another dialog. `ProfileModal` passes it so the dialog Paper's brand gradient doesn't get repainted by `ProfileLayout`. Paper reproduces ContentArea gutters (spacing(2) mobile, spacing(4) ≥md) because the embedded surface skips `ContentArea`.
- **Mobile drawer (`ProfileMobileMenu`) is intentionally right-anchored at 320px with the profile gradient bg** — Figma 322:49246 shows a fullscreen 390 left-anchor variant but the prior product decision was to use a narrower side drawer. Typography fixes from Figma apply; width/anchor/background do NOT — touch only with explicit re-approval.
- `My Places` queries BOTH `/places?owner=` and `/worlds?owner=` from `places-api` and merges them. Worlds use `world_name` for the jump URL (`/jump/places?realm=<name>`), LAND uses `base_position`. See `features/profile/profile.places.client.ts`.
