...
import { CONTENT_POLICY } from '../src/data/legal-pages'
import type { LegalPageDefinition } from '../src/data/legal-pages'
...
const ALLOWED_ROOT_PATHS = ['/blog', '/reels', '/events', '/jump', '/content'] as const

const LEGAL_PAGES: Record<string, LegalPageDefinition> = {
  [CONTENT_POLICY.slug]: CONTENT_POLICY
}

const legalPageToHtml = (page: LegalPageDefinition): string => {
  if (!page.sections.length) return ''
  const sectionHtml = page.sections
    .map(s => `<section id="${s.id}"><h2>${s.title}</h2>${s.paragraphs.map(p => `<p>${escapeHTMLTextContent(p)}</p>`).join('')}</section>`)
    .join('')
  return `<article><h1>${escapeHTMLTextContent(page.title)}</h1>${sectionHtml}</article>`
}
