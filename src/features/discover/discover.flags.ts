...
// What the LIVE section requires when the flag is off, absent, or carries no usable number: no cut
// at all, so the rail is the four busiest scenes the feed returns even when nobody is in them. The
// variant is where product raises the bar; a failed flag fetch must never make the rail stricter
// than that.
/** @internal — exported for testing (see discover.flags.spec.ts); not part of this module's public contract. */
const DEFAULT_LIVE_MIN_USERS = 0
 ...
 * does), and the payload is a string an operator typed into Unleash, so only a plain non-negative
 * integer counts ("0" means no cut): whitespace around it is tolerated (the live file already ships
 * payloads with a trailing newline), but "2.5", "1e3" or "5px" fall back to DEFAULT_LIVE_MIN_USERS
 * rather than being guessed.
 */
function useLiveMinUsers(): number {
  const raw = useRemoteFeatureFlagVariant(FEATURE_FLAG.placesLiveMinUsers)?.trim()
  return raw !== undefined && /^\d+$/.test(raw) ? Number(raw) : DEFAULT_LIVE_MIN_USERS
}
