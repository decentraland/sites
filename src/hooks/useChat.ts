...
const useChat = () => {
  ...
  // NOTE: this hook is deliberately read-only — every chat surface (cast +
  // discover) renders messages without a send input; participation happens
  // in-world. The RFC-4 send path was removed with the inputs; restore it from
  // git history if a send surface ever returns.
  return { chatMessages }
}
