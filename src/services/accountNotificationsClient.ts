...
const getNotificationsApiUrl = (): string => {
  const url = getEnv('NOTIFICATIONS_API_URL')
  if (!url) throw new Error('NOTIFICATIONS_API_URL environment variable is not set')
  return url
}

const signedFetch = signedFetchFactory()

const accountNotificationsBaseQuery: BaseQueryFn<string | (FetchArgs & { baseUrl?: string }), unknown, FetchBaseQueryError> = async (
  args,
  api,
  extraOptions
) => {
  try {
    ...
    const baseUrl = customBaseUrl ?? getNotificationsApiUrl()
    ...
    const fetchFn: typeof fetch = async (input, init) => {
      ...
      // wallet from a previous session and sign subscription calls as the wrong user.
...
const accountNotificationsClient = createApi({
  ...
  baseQuery: accountNotificationsBaseQuery,
