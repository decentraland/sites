    ...
    <CreatorsHeroSection ref={sectionRef}>
      <CreatorsHeroWrapper>
        <CreatorsHeroTextContainer>
          ...
          <CreatorsHeroSubtitle variant="h3" sx={{ typography: { sm: 'h4' } }}>
            {item.subtitle}
          </CreatorsHeroSubtitle>
          <CreatorsHeroActionsContainer>
            {isDesktop && !isLoadingLinks && links && !isLoadingUserAgentData && (
              <DownloadOptions userAgentData={userAgentData} links={links} redirectPath="/download/creator-hub-success" hideLogo />
            )}
            {!isDesktop && (
              <DownloadButton
