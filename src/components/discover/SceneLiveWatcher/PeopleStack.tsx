...
interface PeopleStackProps {
  // Places-api `user_count` snapshot. Shown while the LiveKit room is still
  // connecting so the pill never advertises a misleading "0 people" before
  // we know better. Drops out as soon as real participants resolve.
  initialCount?: number
}
...
function PeopleStack({ initialCount }: PeopleStackProps = {}) {
  ...
  if (total === 0) {
    // While LiveKit is still resolving participants, fall back to the
    // places-api `user_count` snapshot so the pill shows a realistic number
    // immediately instead of "0 people" for the seconds it takes the room
    // to connect. Pill is non-interactive until we have real participants
