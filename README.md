...
| Discover                   | `/discover` (LIVE + BROWSE), `/discover/communities`, `/discover/communities/:id`, `/discover/place/:position`, `/discover/world/:name`, `/discover/*` (404) | Heavy DappsShell route. LiveKit scene watcher + in-world chat + presence stack, communities list/detail. Legacy `/social/*` paths redirect here.                                                                                                       |
...
| Creators                   | `/creators` (Creations grid), `/creators/world/:name` + `/overview` `/preview` `/deploys` `/storage` `/analytics` `/monetization`, `/creators/*` (404)       | Heavy DappsShell route. Roblox-style dashboard for Worlds with a routed left rail (each tab its own page): overview, live bevy preview (reuses the Discover watcher), deploys, embedded scene/env/player storage, analytics/monetization placeholders. |
...
Environment values live in `src/config/env/{dev,stg,prd}.json` and are accessed via `getEnv('KEY')` from `src/config/env.ts`. The `@dcl/ui-env` package auto-selects the right file based on the hostname; override at runtime with `?env=dev|stg|prod` (the value is the `@dcl/ui-env` Env value, not the filename — `?env=prod` loads `prd.json` / the `.org` cluster).
