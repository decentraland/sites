...
| Legal                      | `/brand`, `/content`, `/credits-terms`, `/ethics`, `/privacy`, `/referral-terms`, `/rewards-terms`, `/security`, `/terms`                     | Static legal pages.                                                                                                                                                                         |
...
| What's on (events)         | `/events`, `/events/new-hangout`, `/events/edit-hangout/:eventId`, `/events/admin/pending-events`, `/events/admin/users`                      | Heavy DappsShell route. Legacy `/events/new-event` and `/events/edit-event/:eventId` redirect into the hangout flow; `/events/*` and `/places/*` paths redirect here with deep-link params. |
...
| Discover (explore)         | `/places` (Live Now + Featured + Explore), `/places/communities`, `/places/place/:position`, `/places/world/:name`, `/places/*` (404)         | Heavy DappsShell route. Live scene preview (bevy embed + read-only LiveKit chat), places/worlds explore grid, communities list (detail links into `/social/communities/:id`).               |
...
Environment values live in `src/config/env/{dev,stg,prd}.json` and are accessed via `getEnv('KEY')` from `src/config/env.ts`. The `@dcl/ui-env` package auto-selects the right file based on the hostname; override at runtime with `?env=dev|stg|prod` (note: `prod`, not `prd` — the file is `prd.json` but the query value must match `@dcl/ui-env`'s `Env.PRODUCTION = "prod"`; an invalid value silently falls back to dev).
