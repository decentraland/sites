...
| What's on (events)         | `/events`, `/events/new-event`, `/events/edit-event/:eventId`, `/events/admin/pending-events`, `/events/admin/users`                          | Heavy DappsShell route. Legacy `/events/new-hangout` and `/events/edit-hangout/:eventId` redirect into the event flow; `/events/*` and `/places/*` paths redirect here with deep-link params. |
