...
- **First-party proxy:** ad-blocker filter lists match `cdn.segment.com` and `api.segment.io`, so those sessions send nothing. Two optional env keys move Segment to `evs.e.decentraland.org` (`src/modules/segmentConfig.ts`): `SEGMENT_CDN_URL` (origin for the settings fetch and the GA4 remote plugin, passed to the SDK as `cdnUrl`) and `SEGMENT_API_HOST` (host + base path for event delivery, also the base of the beacon's track URL via `getSegmentTrackUrl()`). Both are validated — a value that is not an absolute https URL is dropped with a warning and Segment's own hosts are kept. **`SEGMENT_API_HOST` is unset everywhere today**: the proxy serves the CDN but 404s on `/v1/t` and `/v1/track`, so setting it would drop every event, SDK-sent and beacon alike.
