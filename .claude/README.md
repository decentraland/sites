...
| ----------------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PreToolUse(Bash)                    | `pre-bash-block-destructive.sh`  | Blocks `npm install --package-lock-only` (rule 21), `git commit --no-verify` (Pre-commit checks), force-push to `origin master`, `git reset --hard`, `git clean -fd`, `git branch -D master`, and `rm -rf` on critical paths (`src/intl`, `src/shells`, `src/features`, `src/components`, `src/hooks`, `src/services`, `src/pages`, `.husky`, `.github`, `api`, `public`, `scripts`, `docs`). Permits the canonical lockfile-reset flow (`rm -rf node_modules package-lock.json && npm install`) per rule 21. |
