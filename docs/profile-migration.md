...
<!-- Code-reviewer P0/P1 — addressed in commit `3a75688`. Kept here as resolved markers; remove on next plan refresh once Jarvis re-reviews and confirms. -->
- ✅ **CSS URL injection** (P0 — `PlacesTab.tsx:59`). `safeCssUrl` hoisted to `src/utils/safeCssUrl.ts`; `PlaceImage` now takes a `$image` transient prop with the validated URL.
- ✅ **Friends modal anti-stacking** (P1 — `ProfileHeader.tsx`). `ProfileHeader` gained an `embedded?: boolean` prop (plumbed from `ProfileSurface`); when true it hides the friends-count button and skips the `<FriendsModal>` mount entirely.
- ✅ **Raw RTK Query error body leak** (P1 — `profile.places.client.ts`). Dropped `data: places.body`; log raw via `console.error('[ProfilePlaces] /places failed', ...)` and surface a generic `error: 'Places fetch failed'` (same for `/worlds`).
- ✅ **List card not memoized** (P1 — `CommunityCardItem` in `CommunitiesTab.tsx`). Wrapped in `memo()`. `handleOpenCommunity` is now stable `(id, event) => void`; the card builds the per-id closure once via internal `handleAnchorClick`.
- ✅ **Singleton RPC client race** (P1 — `profile.social.rpc.ts`). Replaced the single `connectPromise` with `inflight: Map<key, Promise<Client>>` keyed by identity; competing identities no longer disconnect each other's freshly-resolved clients.
- Phase 13 — Automated E2E + Lighthouse audit + bundle-size diff vs master.
