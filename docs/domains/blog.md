...
| `src/services/cmsClient.ts`           | RTK Query base — `cmsClient` (Contentful + cms-server search). Empty endpoints; injected from `features/cms/`. Filename is mildly misleading — a future rename to `services/cmsClient.ts` (already correct) is fine if you spot it. |
